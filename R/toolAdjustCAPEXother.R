...
toolAdjustCAPEXother <- function(dt, ISOcountries, yrs, completeData, GDPpcMER) {
  ...
  dt[subsectorL1 %in% c("Freight Rail", "Passenger Rail", "HSR"), value := value * 0.4]
  dt[subsectorL1 %in% c("Freight Rail", "Passenger Rail", "HSR"), variable := "CAPEX"]
  # Ships
  # CCS ships doi:10.1016/j.egypro.2014.11.285
  # CAPEX ~ 30%
  dt[subsectorL1 %in% c("Domestic Ship", "International Ship"), value := value * 0.3]
  dt[subsectorL1 %in% c("Domestic Ship", "International Ship"), variable := "CAPEX"]
