...
readTRACCS <- function(subtype = c("fuelEnDensity", "roadFeDemand", "energyIntensity", "loadFactor", "annualMileage",
                                   "roadVkmDemand", "roadTkmDemand", "roadPkmDemand", "railFeDemand", "vehPopulation")) {
  `.` <- iso <- period <- categoryTRACCS <- vehicleType <- technology <- value <- NULL
  subtype <- match.arg(subtype)

  countries <- list.files(
    path = file.path("./TRACCS/TRACCS_ROAD_Final_EXCEL_2013-12-20"),
    all.files = FALSE)
  ...
  switch(
      ...
      data <- rbindlist(lapply(
        ...
        function(x) {
          conv <- suppressMessages(data.table(read_excel(
            path = file.path(
              "TRACCS/TRACCS_ROAD_Final_EXCEL_2013-12-20",
    ...
    "railFeDemand" = {
      data <- suppressMessages(data.table(read_excel(
        path = "TRACCS/TRACCS_RAIL_Final_EXCEL_2013-12-20/TRACCS_Rail_Final_Eval.xlsx",
