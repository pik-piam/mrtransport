...
calcEdgeTransportSAinputs <- function(subtype, adjustments = TRUE) {
  ...
  setkey(completeDataSet, region,  sector, subsectorL1, subsectorL2, subsectorL3, vehicleType, technology, univocalName, period)
  ...
  #(2) GCAM:
  #- Includes regionally differentiated data for 32 regions globally (covers all iso countries)
  ...
  #- Timewise only a value for 2015 and for 2040 is given
  ...
  switch(
    ...
    "energyIntensity" ={
      ...
      energyIntensityRawGCAMconventionalCarsnonTRACCS <- data$enIntGCAM[subsectorL3 == "trn_pass_road_LDV_4W" & technology %in% c("Liquids", "NG") & !region %in% countriesTRACCS]
      #- All other data for non-TRACCS countries except for Trucks
      energyIntensityRawGCAMnonCarsnonTRACCS <- data$enIntGCAM[!subsectorL3 == "trn_pass_road_LDV_4W" & !subsectorL1 == "trn_freight_road" & !region %in% countriesTRACCS]
      ...
      energyIntensityRawGCAMalt2WheelersTRACCSreg <- data$enIntGCAM[subsectorL3 == "trn_pass_road_LDV_2W" & region %in% countriesTRACCS & technology == "BEV"]
      ...
      energyIntensityRawPSIalternativeTechTRACCSreg <- merge(energyIntensityRawPSIalternativeTechTRACCSreg, TRACCSVehTypes, by = c("region", "sector", "subsectorL1", "subsectorL2", "subsectorL3", "vehicleType",
      ...
      #Large Car is missing for some nonTRACCS regions, filled in toolAdjustEnergyIntensity -> NAs are introduced for unit & variable
      energyIntensityRawPSIalternativeCarsnonTRACCS[, variable := "Energy intensity"][, unit := "MJ/vehkm"]

      energyIntensityRaw <- rbind(data$enIntTRACCS, energyIntensityRawGCAMconventionalCarsnonTRACCS, energyIntensityRawGCAMmissingTRACCScat, energyIntensityRawGCAMnonCarsnonTRACCS, energyIntensityRawGCAMalt2WheelersTRACCSreg,
                                  energyIntensityRawPSITrucks, energyIntensityRawPSItrucksNGTRACCSreg, energyIntensityRawPSIalternativeTechTRACCSreg, energyIntensityRawPSIalternativeCarsnonTRACCS)

      setkey(energyIntensityRaw, region,  sector, subsectorL1, subsectorL2, subsectorL3, vehicleType, technology, period, univocalName)
    ...
    "histESdemand" = {
      ...
      enIntensity <- magpie2dt(calcOutput(type = "EdgeTransportSAinputs", subtype = "energyIntensity", warnNA = FALSE, aggregate = FALSE))[, unit := NULL][, variable := NULL]
      loadFactor <- magpie2dt(calcOutput(type = "EdgeTransportSAinputs", subtype = "loadFactor", warnNA = FALSE, aggregate = FALSE))[, unit := NULL][, variable := NULL]
      ...
      esDemandEUROSTAT <- merge(data$feDemandEUROSTAT, enIntensity, by = c("region",  "sector", "subsectorL1", "subsectorL2", "subsectorL3", "vehicleType", "technology",
                                                                           "period", "univocalName"))
      esDemandEUROSTAT <- merge(esDemandEUROSTAT, loadFactor, by = c("region",  "sector", "subsectorL1", "subsectorL2", "subsectorL3", "vehicleType", "technology",
                                                                     "period", "univocalName"))
      ...
      setkey(esDemand, region,  sector, subsectorL1, subsectorL2, subsectorL3, vehicleType, technology, period, univocalName)
   ...
   "loadFactor" = {
    ...
    "CAPEXtrackedFleet" = {
      ...
      unit <- "US$2005/veh"
      description <- "CAPEX (purchase costs) for vehicle types that feature fleet tracking (cars, trucks and busses). Sources: UCD, PSI"
      weight <- calcOutput("GDP", aggregate = FALSE)[, years, "gdp_SSP2"]
      ...
      annualMileage <-  magpie2dt(calcOutput(type = "EdgeTransportSAinputs", subtype = "annualMileage",  warnNA = FALSE, aggregate = FALSE))
      setnames(annualMileage, "value", "annualMileage")
      annualMileage[, c("unit", "variable") := NULL]
      ...
      CAPEXUCD4W[, .(value = sum(value)), by = c("region",  "sector", "subsectorL1", "subsectorL2", "subsectorL3", "vehicleType", "technology", "univocalName", "unit", "period")]
      ...
      CAPEX <- toolAdjustCAPEXtrackedFleet(CAPEXraw, ISOcountriesMap, years)
    ...
    "nonFuelOPEXtrackedFleet" = {
      ...
      unit <- "US$2005/veh/yr"
      description <- "Non-fuel OPEX on technology level for vehicle types that feature fleet tracking (cars, trucks, busses). Sources: UCD, PSI"
      weight <- calcOutput("GDP", aggregate = FALSE)[, years, "gdp_SSP2"]
      ...
      nonFuelOPEXcombinedUCD <- merge(CAPEXcombinedUCD, annualMileage, all.x = TRUE, by = c("region",  "sector", "subsectorL1", "subsectorL2", "subsectorL3", "vehicleType",
      ...
      completeDataSet <- completeDataSet[subsectorL1 == "trn_freight_road" | subsectorL3 == "trn_pass_road_LDV_4W" | subsectorL2 == "Bus"]
      #Check whether data is complete
      check <- merge(completeDataSet, nonFuelOPEX, all = TRUE)
      if (nrow(check[is.na(value)]) > 0) {
