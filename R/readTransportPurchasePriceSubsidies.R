...
readTransportPurchasePriceSubsidies <- function() {
  ...
  data <- data.table(read_excel(
    # TODO: This needs to be fixed: I cannot keep my local path! (done only because file_path kept failing)
    #path = "C:/Users/caroline/Documents/edgeTransportInputDataCluster/sources/TransportSubsidiesUpdated11_23/carSubsidies.xlsx",
    path = file.path("carSubsidies.xlsx"),
    sheet="RoundedSubsidiesUSD2005","A1:K97"))
  data <- melt(data, id.vars = c("region", "subsidiesVehicleType", "subsidiesTechnology", "untilPrice20", "untilPrice21", "untilPrice22", "untilPrice23"), 
  ...
  # find duplicates. This happens when a subsidy is granted below a price of A, and a different subsidy is granted for a price greater than A but below B
  ...
  # we DONT want this
  data <- melt(data, id.vars = c("region", "subsidiesVehicleType", "subsidiesTechnology", "period"), measure.vars = c("untilPrice", "purchasePriceSubsidy", "untilPrice2", "purchasePriceSubsidy2"), value.name = "value")
