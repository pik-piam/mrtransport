...
#' @importFrom magrittr `%>%`
#' @export
readPSI <- function(subtype = c("costs", "energyIntensity")) {
  switch(
    subtype,
    "costs" = {
      dt <- suppressMessages(data.table(read_excel("PSI/Car model result_modified.xlsx", ##file is modified from the original one: names are copy pasted to simplify R data wrangling
      ...
      dt[, variable := "total purchase costs"][, unit := "2017EUR/veh"][, region := "GLO"]
    ...
    "energyIntensity" = {
      ...
      dt_LDV <- fread("PSI/ttw-efficiencies.csv")
      ...
      dt_Trucks <- fread("PSI/Regional delivery_truck_efficiencies.csv")[, V1 := NULL]
