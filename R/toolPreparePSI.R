...
toolPreparePSI <- function(x) {
  variable <- . <- technology <- period <- value <- univocalName <- unit <- region  <- vehicleTypePSI <-
    technologyPSI <- NULL

  mapfile <- system.file("extdata", "mappingPSItoEDGET.csv", package = "mrtransport", mustWork = TRUE)
  ...
  dt <- merge.data.table(dt,
                         mappingPSI,
                         all.x = TRUE,
                         by = c("technologyPSI", "vehicleTypePSI"),
